...
import { useGetDisciplineRecordsQuery, useAddDisciplineRecordMutation } from "../features/healthDiscipline/healthDisciplineApi";
...
const DisciplineRecords = () => {
  ...
  const { data, isLoading, error } = useGetDisciplineRecordsQuery(studentId);
  const [addDisciplineRecord] = useAddDisciplineRecordMutation();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await addDisciplineRecord({ ...form, studentId });
    setForm({ date: "", details: "" });
  };

  return (
    <Box p={2}>
      <Typography variant="h5" gutterBottom>Student Discipline Records</Typography>
      <Paper sx={{ p: 2, mb: 2 }}>
